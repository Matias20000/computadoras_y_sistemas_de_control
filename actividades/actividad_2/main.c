...
i2c_init(i2c0, 100 1000);// Elijo GPIO4 como linea de SDA
gpio_set_function(SDA_GPIO, GPIO_FUNC_I2C);// Elijo GPIO5 como linea de SCL
gpio_set_function(SCL_GPIO, GPIO_FUNC_I2C);// Activo pull-up en ambos GPIO, son debiles por lo que// es recomendable usar pull-ups externas
gpio_pull_up(SDA_GPIO);
gpio_pull_up(SCL_GPIO);// Inicializo LCD
lcd_init();// Limpio LCD
lcd_clear();// Escribo lo basico
lcd_string("TEMP=");
lcd_set_cursor(1, 0);
lcd_string("SETP=");// Inicializo ADC
adc_init();// Inicializo el GPIO del NTC como entrada analogica
adc_gpio_init(NTC_GPIO);// Inicializo el GPIO del potenciometro como entrada analogica
adc_gpio_init(SP_GPIO);// Inicializacion de PWM// - 100 Hz de frecuencia// - Ancho de pulso inicial del 0%
